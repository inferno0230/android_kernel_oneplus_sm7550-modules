		...
		struct dsi_panel_cmd_set *cmdset)
{
	...
	int rc = 0;
	struct dsi_panel_cmd_set local_cmdset;
	struct dsi_cmd_desc *local_cmd = NULL;
	struct iris_cfg *pcfg = iris_get_cfg();
	if (!cmdset || !panel) {
	...
	memset(&local_cmdset, 0x00, sizeof(local_cmdset));
	local_cmd = kvzalloc(cmdset->count * sizeof(struct dsi_cmd_desc), GFP_KERNEL);
	if (!local_cmd) {
		IRIS_LOGE("%s(), failed to alloc desc cmd", __func__);
		return -ENOMEM;
	}
	memcpy(local_cmd, cmdset->cmds, cmdset->count * sizeof(struct dsi_cmd_desc));
	memcpy(&local_cmdset, cmdset, sizeof(local_cmdset));
	local_cmdset.cmds = local_cmd;

	{
		int32_t i = 0;
		struct dsi_cmd_desc *cmds = local_cmdset.cmds;
		enum dsi_cmd_set_state state = local_cmdset.state;
	...
	if (local_cmdset.count == 1 && iris_is_read_cmd(&local_cmdset.cmds[0])) {
		ktime_t lp_ktime0;
		if ((iris_esd_ctrl_get() & 0x8) || IRIS_IF_LOGD()) {
			lp_ktime0 = ktime_get();
		}
		rc = iris_pt_read_panel_cmd(panel, &local_cmdset);
		if ((iris_esd_ctrl_get() & 0x8) || IRIS_IF_LOGD()) {
	...
	if (!pcfg->vc_ctrl.vc_enable) {
		...
		vc_id = (local_cmdset.state == DSI_CMD_SET_STATE_LP) ?
			pcfg->vc_ctrl.to_panel_lp_vc_id :
			pcfg->vc_ctrl.to_panel_hs_vc_id;
		IRIS_LOGD("using aux channel %d", vc_id);
		iris_dsi_send_cmds(panel, local_cmdset.cmds,
			local_cmdset.count, DSI_CMD_SET_STATE_HS, vc_id);
	}
